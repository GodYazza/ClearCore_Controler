...
Version: Mk1.0
Last Edit: 01/11/2022

The purpose of this code is to operate the stepper/servo motor drivers and subsequent motors which operate the 
XYZ axis movement of the StarDestroyer IPT test rig located in the high-risk labs in the Newmarket campus.
...
ClearCore controller. This code can also be used to provide basic move commands to a TEKNIC 
servo motor driver (like that uses on the Z axis mover, and in Jack's EV lab).
...
// Global Variables
/************** GLOBAL VARIABLES **************/
int32_t global_counts_per_mm = 500;
int32_t global_X_axis_counts_per_mm = 200;  // 200 counts per motor revolution, into a 5 to 1 worm drive, then into a 5mm per turn (5mm-pitch) screw
...
int32_t Velocity_Limit_M0_X = 2000; // pulses/counts per sec, maximum tried with no VA is 20,000 counts per second
int32_t Acceleration_Limit_M0_X = 20000; // pulses/counts per sec^2

// Define the velocity and acceleration limits to be used for each move
int32_t Homing_Velocity_Limit_M0_X = 2000; // pulses/counts per sec
		...
		*/
		...
		if (global_command_received_flag == 1) { // STATE 2: EXECUTING COMMAND
			...
			if (strcmp(global_python_command, "HomeX#") == 0) { // STATE 3: HOMING X
				...
				// Python Control has requested a HomeZ command, so we need to execute it
				Move_Home_M0_X();
			...
			if (strcmp(global_python_command, "MoveXForward#") == 0) { // STATE 4: Moving X to forward sensor at speed
				IO0_OFF
				IO1_OFF 
				IO2_ON // 0010 (STATE 4)
				global_command_received_flag = -1; // Reset the flag and erase the command as we are about to execute the command
...
void Initialization_General() {
	...
	Emergency_Stop_InputA10.InterruptHandlerSet(Emergency_Stop_A10_ISR, InputManager::RISING);
...
/*------------------------------------------------------------------------------
 * Move_Home_M0_X
 *	  Homing function that causes the motor to move in the reverse direction
 *    Returns when motor has homed to DI-7
 *
 *
 * Returns: None
 */
void Move_Home_M0_X() {
	...
	Neg_Limit_M0_X_DI7.InterruptHandlerSet(Homing_M0_Z_DI6_ISR, InputManager::RISING);
...
void Move_to_Forward_M0_X(){
	...
	// Sets the ISR for DI6 to now trigger the homing ISR
	Pos_Limit_M0_X_DI6.Mode(Connector::INPUT_DIGITAL);
	Pos_Limit_M0_X_DI6.InterruptHandlerSet(Moving_M0_X_DI6_ISR, InputManager::RISING);
	
	// Tells the motor to move in the positive direction indefinitely
	motor_M0_X.MoveVelocity(-Homing_Velocity_Limit_M0_X);
	
	while (true){
		if (Forward_Sensor_Flag == 1){
			break;
		}
	}
	motor_M0_X.VelMax(Velocity_Limit_M0_X);
	...
	SerialPort.SendLine("CLEARCORE: Move To Forward Sensor Complete");
	Intialization_ISR_Limit_Switches();
	Delay_ms(1000);
